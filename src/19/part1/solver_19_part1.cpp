...
struct Beacon
{
  ...
  {
    return lhs.u == rhs.u and lhs.v == rhs.v and lhs.w == rhs.w;
  }
  friend bool operator!=(const Beacon &lhs, const Beacon &rhs)
  {
    return !(lhs == rhs);
  ...
  friend bool operator<(const Beacon &lhs, const Beacon &rhs)
  {
    const auto l_magnitude = lhs.u * lhs.u + lhs.v * lhs.v + lhs.w * lhs.w;
    const auto r_magnitude = rhs.u * rhs.u + rhs.v * rhs.v + rhs.w * rhs.w;
  ...
  struct Hasher
  {
    ...
    {
      size_t result = 17;
      result = result * 31 + std::hash<int>()(beacon.u);
      result = result * 31 + std::hash<int>()(beacon.v);
      result = result * 31 + std::hash<int>()(beacon.w);
      return result;
    }
  };
};
...
{
  return {
    p.u * xu(a, b) + p.v * yu(a, b, y) + p.w * zu(a, b, y),
    p.u * xv(a, b) + p.v * yv(a, b, y) + p.w * zv(a, b, y),
    p.u * xw(b) + p.v * yw(b, y) + p.w * zw(b, y),
  };
}
